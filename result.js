document.addEventListener('DOMContentLoaded', () => {
    ...
    // 초기 제품 배열
    const products = [
        { name: '상품 1', image: 'top/1.png', brand: '브랜드 1', price: '₩50,000', discount: '20%' },
        { name: '상품 2', image: 'bottom/1.png', brand: '브랜드 2', price: '₩60,000', discount: '15%' },
        { name: '상품 3', image: 'outer/1.png', brand: '브랜드 3', price: '₩70,000', discount: '10%' }
    ];    

    // CSV 파일을 불러오고 id, image, name, brand, price, discount 값을 업데이트하는 함수
    function loadProductIdsFromCSV() {
        const csvFilePath = 'recommend1.csv'; // CSV 파일 경로
        ...
        Papa.parse(csvFilePath, {
            ...
            complete: function(results) {
                ...
                products.forEach((product, index) => {
                    if (csvData[index]) {
                        product.id = csvData[index]['Old File Name'];  // 'Old File Name' 열을 id로 사용
                        product.image = csvData[index]['New File Name'];  // 'New File Name' 열을 image로 사용
    ...
    function loadProducts() {
        products.forEach((product, index) => {
            ...
            document.getElementById(`product-image-${index}`).src = `recommend_images_output/rec${product.image}.png`;
